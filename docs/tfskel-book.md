...
- `workflows.name_template`: Plain string stem for the per-env workflow filename (optional)
  - **Go template syntax is NOT supported**
  - The environment prefix and `.yaml` extension are added automatically
  - Example: `name_template: "terraform"` + `--env dev` → `dev-terraform.yaml`
  - Default: `"terraform"`
...
- `<env>-<name_template>.yaml` - Terraform plan/apply caller for a specific environment
...
6. Embeds metadata in generated files for change detection
...
3. Writes `.github/workflows/<env>-<name_template>.yaml`

> [!NOTE]
> `workflows.name_template` is a **plain string** — Go template syntax is not supported. The env prefix and `.yaml` extension are added automatically. Example: `name_template: "terraform"` + `--env dev` → `dev-terraform.yaml`.
