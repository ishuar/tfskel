...
```hcl
## tfskel-source: {"template":"backend.tf.tmpl","hash":"a1b2c3d4e5f6..."}
```

For YAML files the comment prefix is `#` instead of `##`:

```yaml
# tfskel-source: {"template":"lint.yaml","hash":"f6e5d4c3b2a1..."}
```

The marker records:
- **template**: Which template produced the file
- **hash**: SHA-256 hash of the rendered content at generation time
...
5. **Compare** the new hash against the stored hash
6. **Overwrite** the file only if content has changed

This means `--upgrade` is safe to run repeatedly — files already up to date are left untouched.
