...
## tfskel-tags-hash: c4fbfbd742863959
provider "aws" {
  ...
  default_tags {
    tags = {
      ...
      app = "myapp"
		...
		assert.NoError(t, err)
		assert.True(t, needsUpdate)
		assert.Contains(t, changes[0], "default_tags changed")
	})
...
terraform {
  ...
  required_version = "~> 1.13"
	...
	t.Run("returns false when file has old tfskel-tags format and config has no tags", func(t *testing.T) {
		t.Helper()
		...
		// File with old JSON-based tfskel-tags (not the new hash format)
		versionsContent := `## tfskel-metadata: {"tf_ver": "~> 1.13", "aws_provider_ver": "~> 6.0"}
  ...
  required_providers {
    aws = {
      ...
      version = "~> 6.0"
		...
		needsUpdate, _, err := gen.shouldUpdateVersions("versions.tf", data)
		assert.NoError(t, err)
		assert.False(t, needsUpdate,
			"No hash found + no config tags = no update needed")
	})

	t.Run("returns true when file has no tags hash and config has tags", func(t *testing.T) {
		t.Helper()
		...
		// File with old JSON-based tfskel-tags (no tfskel-tags-hash)
		versionsContent := `## tfskel-metadata: {"tf_ver": "~> 1.13", "aws_provider_ver": "~> 6.0"}
  ...
  required_providers {
    aws = {
      ...
      version = "~> 6.0"
		...
		assert.NoError(t, err)
		assert.True(t, needsUpdate)
		assert.Contains(t, changes[0], "initialized tags tracking",
			"Should initialize tags tracking when no hash found but config has tags")
	})
...
func TestUpgradeFileIfEligible(t *testing.T) {
	...
	t.Run("skips file without source marker when force is false", func(t *testing.T) {
		filesystem := fs.NewMemoryFileSystem()
		log := logger.New(false)
		gen := NewGenerator(baseCfg, filesystem, log)
		...
		// Write a file without source marker
		outputPath := "envs/dev/us-east-1/myapp/backend.tf"
		_ = filesystem.MkdirAll("envs/dev/us-east-1/myapp", 0755)
		_ = filesystem.WriteFile(outputPath, []byte("# no marker here\nterraform {}"), 0644)

		data := &templates.Data{Env: "dev", Region: "us-east-1"}
		err = gen.upgradeFileIfEligible("tf/backend.tf.tmpl", outputPath, data)
		require.NoError(t, err)

		// File should not have been overwritten
		content, _ := filesystem.ReadFile(outputPath)
		assert.Contains(t, string(content), "# no marker here")
	})

	t.Run("overwrites file without source marker when force is true", func(t *testing.T) {
		filesystem := fs.NewMemoryFileSystem()
		log := logger.New(false)
		gen := NewGenerator(baseCfg, filesystem, log)
		gen.SetUpgrade(true, true)
		...
		outputPath := "envs/dev/us-east-1/myapp/backend.tf"
		_ = filesystem.MkdirAll("envs/dev/us-east-1/myapp", 0755)
		_ = filesystem.WriteFile(outputPath, []byte("# old content without marker"), 0644)
		...
		// File should have been re-rendered (contains source marker now)
		content, _ := filesystem.ReadFile(outputPath)
		assert.Contains(t, string(content), "tfskel-source")
	})

	t.Run("skips file when rendered content matches", func(t *testing.T) {
		filesystem := fs.NewMemoryFileSystem()
		log := logger.New(false)
		gen := NewGenerator(baseCfg, filesystem, log)
		gen.SetUpgrade(true, false)
		...
		// First generate the file normally so its content matches what re-render would produce
		tmplName := "tf/backend.tf.tmpl"
		outputPath := "envs/dev/us-east-1/myapp/backend.tf"
		_ = filesystem.MkdirAll("envs/dev/us-east-1/myapp", 0755)
		...
		contentBefore, _ := filesystem.ReadFile(outputPath)
		err = gen.upgradeFileIfEligible(tmplName, outputPath, data)
		require.NoError(t, err)

		contentAfter, _ := filesystem.ReadFile(outputPath)
		assert.Equal(t, string(contentBefore), string(contentAfter))
	})

	t.Run("re-renders file when content was manually edited", func(t *testing.T) {
		filesystem := fs.NewMemoryFileSystem()
		log := logger.New(false)
		gen := NewGenerator(baseCfg, filesystem, log)
		gen.SetUpgrade(true, false)
		...
		// Write file with correct source marker but manually edited content
		tmplName := "tf/backend.tf.tmpl"
		currentHash := renderer.GetTemplateHash(tmplName)
		markerContent := fmt.Sprintf(`## tfskel-source: {"template":"%s","hash":"%s"}`, tmplName, currentHash)
		fileContent := markerContent + "\n# manually edited content that differs from template"

		outputPath := "envs/dev/us-east-1/myapp/backend.tf"
		_ = filesystem.MkdirAll("envs/dev/us-east-1/myapp", 0755)
		_ = filesystem.WriteFile(outputPath, []byte(fileContent), 0644)

		data := &templates.Data{Env: "dev", Region: "us-east-1", S3BucketName: "my-bucket", ShortRegion: "use1"}
		err = gen.upgradeFileIfEligible(tmplName, outputPath, data)
		require.NoError(t, err)

		// File should have been re-rendered (content drift detected)
		content, _ := filesystem.ReadFile(outputPath)
		assert.NotContains(t, string(content), "manually edited content")
		assert.Contains(t, string(content), "tfskel-source")
	})

	t.Run("re-renders file when template hash differs", func(t *testing.T) {
		filesystem := fs.NewMemoryFileSystem()
		log := logger.New(false)
		gen := NewGenerator(baseCfg, filesystem, log)
		gen.SetUpgrade(true, false)
		...
		// Write file with outdated hash
		tmplName := "tf/backend.tf.tmpl"
		markerContent := fmt.Sprintf(`## tfskel-source: {"template":"%s","hash":"outdated_hash_value"}`, tmplName)
		fileContent := markerContent + "\nold content"
		...
		// File should have been re-rendered with new content and updated hash
		content, _ := filesystem.ReadFile(outputPath)
		assert.NotContains(t, string(content), "outdated_hash_value")
		assert.Contains(t, string(content), "tfskel-source")
	})

	t.Run("skips when marker template does not match", func(t *testing.T) {
		filesystem := fs.NewMemoryFileSystem()
		log := logger.New(false)
		gen := NewGenerator(baseCfg, filesystem, log)
		gen.SetUpgrade(true, false)
		...
		// Write file with marker from a different template
		markerContent := `## tfskel-source: {"template":"tf/versions.tf.tmpl","hash":"somehash"}`
		fileContent := markerContent + "\n# content from versions template"
		...
		// File should not have changed (template mismatch)
		content, _ := filesystem.ReadFile(outputPath)
		assert.Equal(t, fileContent, string(content))
	})
}

func TestIsUpgradeEligible(t *testing.T) {
	t.Run("all eligible when no whitelist", func(t *testing.T) {
		cfg := &config.Config{Templates: &config.Templates{}}
		gen := NewGenerator(cfg, fs.NewMemoryFileSystem(), logger.New(false))

		assert.True(t, gen.isUpgradeEligible("tf/backend.tf.tmpl"))
		assert.True(t, gen.isUpgradeEligible("tf/versions.tf.tmpl"))
		assert.True(t, gen.isUpgradeEligible("tf/custom.tf.tmpl"))
	})

	t.Run("only whitelisted templates eligible", func(t *testing.T) {
		cfg := &config.Config{
			Templates: &config.Templates{
				Upgrade: []string{"backend.tf.tmpl"},
			},
		}
		gen := NewGenerator(cfg, fs.NewMemoryFileSystem(), logger.New(false))

		assert.True(t, gen.isUpgradeEligible("tf/backend.tf.tmpl"))
		assert.False(t, gen.isUpgradeEligible("tf/versions.tf.tmpl"))
	})

	t.Run("nil templates config means all eligible", func(t *testing.T) {
		cfg := &config.Config{}
		gen := NewGenerator(cfg, fs.NewMemoryFileSystem(), logger.New(false))
...
func TestSetUpgrade(t *testing.T) {
	gen := NewGenerator(&config.Config{}, fs.NewMemoryFileSystem(), logger.New(false))

	assert.False(t, gen.upgrade)
	assert.False(t, gen.force)

	gen.SetUpgrade(true, false)
	assert.True(t, gen.upgrade)
	assert.False(t, gen.force)

	gen.SetUpgrade(true, true)
	assert.True(t, gen.upgrade)
	assert.True(t, gen.force)
}
