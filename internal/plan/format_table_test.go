...
func TestPlanFormatter_writeTableResourceDetails_Filtered(t *testing.T) {
	...
	assert.Contains(t, output, "Filters: severity=critical")
}

func TestPlanFormatter_writeTableResourceDetails_FilterMatchesAll(t *testing.T) {
	formatter := NewPlanFormatterFiltered(false, DefaultTopResourcesCount, 2, []string{"severity=critical"})
	formatter.tableWidth = 100
	...
	output := buf.String()
	assert.Contains(t, output, "Showing 2 resources")
...
func TestPlanFormatter_writeTableResourceDetails_Unfiltered(t *testing.T) {
	...
	output := buf.String()
	assert.Contains(t, output, "Showing 1 resource")
	assert.NotContains(t, output, "filtered")
}
